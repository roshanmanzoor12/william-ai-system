...
/**
 * apps/dashboard/src/components/voice/VoiceWorkerStatusCard.tsx
 *
 * Real Voice Worker device-connector card -- close mirror of
 * apps/dashboard/src/components/system/WindowsWorkerStatusCard.tsx for the
 * voice equivalent. Reads GET /voice/worker/status (voiceWorkerApi.status())
 * and shows exactly what the backend actually knows: connection_state
 * (Needs Setup / Connected / Offline / Disabled), device name, last
 * heartbeat, supported features.
 *
 * "Enable Voice Worker" never pretends a browser click starts a local
 * microphone -- it calls POST /voice/device/setup-token and shows the real,
 * server-generated one-time setup command for the user to run themselves.
 * Only that local run (scripts/windows/install_voice_worker.ps1) can ever
 * flip the state to Connected; this card just reflects reality, polling
 * every 15s, and never fabricates a connected/listening state.
 *
 * Installing/connecting the Voice Worker is a separate concern from voice
 * *mode* (disabled/push_to_talk/wake_word_admin/...) -- that stays
 * VoiceControlSettings.tsx's mode-selector's job. This card intentionally
 * does not let a connected-but-mode-disabled worker look "ready to listen".
 */

import { useEffect, useState } from "react";
import { Check, Copy, Download, Mic, PowerOff, RefreshCw, WifiOff } from "lucide-react";
import {
  API_BASE_URL,
  voiceDeviceSetupApi,
  voiceWorkerApi,
  type VoiceDeviceSetupTokenData,
  type VoiceWorkerStatusData,
} from "@/lib/api-client";
...
const DEV_RUN_COMMAND = [
  ...
  '  --simulate-text "William open Notepad"',
].join("\n");
...
const STATE_STYLES: Record<string, string> = {
  ...
  disabled_device: "bg-neutral-100 text-neutral-500",
...
export function VoiceWorkerStatusCard({ variant = "full" }: Props) {
  const [status, setStatus] = useState<VoiceWorkerStatusData | null>(null);
  const [loading, setLoading] = useState(true);
  const [copied, setCopied] = useState<"setup" | "dev" | null>(null);
  const [setupData, setSetupData] = useState<VoiceDeviceSetupTokenData | null>(null);
  ...
  async function load() {
    const result = await voiceWorkerApi.status();
  ...
  const handleEnable = async () => {
    setEnabling(true);
    setActionError(null);
    const result = await voiceDeviceSetupApi.createSetupToken();
    if (result.success && result.data) {
  ...
  const handleDisable = async () => {
    if (!window.confirm("Disable Voice Worker? The installed worker will stop being able to submit commands until you re-enable it.")) {
      return;
    }
    setDisabling(true);
    setActionError(null);
    const result = await voiceDeviceSetupApi.disable();
    if (result.success) {
      setSetupData(null);
      await load();
    } else {
      setActionError(result.success === false ? result.error.message : "Could not disable Voice Worker.");
    }
    setDisabling(false);
  };
    ...
    <div className="rounded-[1.6rem] bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          >
            {connected ? <Mic size={18} /> : <WifiOff size={18} />}
          </span>
          <div>
            <p className="text-sm font-black text-neutral-900">Voice Worker</p>
        ...
        <>
          ...
          {status?.supported_features && status.supported_features.length > 0 && (
            <div className="mt-4">
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                {status.supported_features.map((feature) => (
                  <span
                    key={feature}
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            {connectionState !== "connected" && (
              ...
              >
                {enabling ? "Generating..." : "Enable Voice Worker"}
              </button>
            )}
            <button
              type="button"
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <a
                    ...
                    download="install_voice_worker.ps1"
