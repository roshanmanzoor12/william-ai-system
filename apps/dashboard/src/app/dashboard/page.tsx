...
function extractAction(command: string): string {
  ...
  if (normalized.includes("code") || normalized.includes("file")) return "code.execute";
  if (normalized.includes("browser") || normalized.includes("website")) return "browser.operate";
  if (normalized.includes("billing") || normalized.includes("subscription")) return "billing.review";
  if (normalized.includes("report") || normalized.includes("analytics")) return "analytics.generate";
  ...
  return (
    <div className="grid min-h-[420px] place-items-center rounded-[2rem] bg-white shadow-sm">
      <div className="text-center">
        ...
        <p className="mt-4 text-sm font-black text-neutral-950">Loading command center...</p>
        <p className="mt-1 text-xs font-medium text-neutral-500">
          Pulling workspace-safe dashboard data.
        </p>
      </div>
    </div>
  );
...
function EmptyPanel() {
  return (
    <div className="grid min-h-[260px] place-items-center rounded-[1.5rem] border border-dashed border-neutral-200 bg-white p-8 text-center">
      <div>
        ...
        <p className="mt-2 max-w-md text-sm font-medium text-neutral-500">
          Run your first command and William will create isolated tasks, audit logs,
          memory context, and verification payloads.
        </p>
      </div>
    </div>
  );
...
export default function Page() {
  ...
  const canRunAgents = useMemo(() => {
    ...
    return hasPermission(session, "agents:run") && hasPermission(session, "tasks:write");
  }, [session]);
  ...
  }, [dashboard.summary.chart.securityCounts, dashboard.summary.chart.taskCounts]);
  ...
  const loadDashboard = useCallback(
    async (activeSession: SessionData) => {
      ...
      const [summaryResponse, agentsResponse, tasksResponse] = await Promise.all([
      ...
      const agents = agentsResponse.success && agentsResponse.data ? agentsResponse.data : [];
      const recentTasks =
  ...
  async function handleCommandSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await apiRequest<CommandResponse>("/master/command", session, {
      method: "POST",
      headers: {
        "X-Action": action,
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[#e9e9e7] px-6 text-neutral-950">
        <div className="rounded-[2rem] bg-white px-8 py-7 text-center shadow-2xl shadow-black/10">
          ...
          <p className="mt-4 text-sm font-black">Checking dashboard session...</p>
          <p className="mt-1 text-xs font-medium text-neutral-500">
          ...
          <header className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
              <div className="lg:hidden flex items-center gap-3 rounded-[1.5rem] bg-white p-3 shadow-sm">
                ...
                <div>
                  <p className="text-sm font-black">William / Jarvis</p>
              ...
              <div className="relative w-full max-w-md">
                ...
                <input
                  type="search"
                  placeholder="Search agents, tasks, approvals, memories..."
          ...
          <div className="mb-6 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm font-medium text-neutral-500">
                Run AI commands, monitor agent flow, track approvals, and verify workspace-safe actions.
              </p>
            </div>
                ...
                <div className="rounded-[1.6rem] bg-white p-5 shadow-sm">
                  ...
                  <div className="h-3 overflow-hidden rounded-full bg-neutral-100">
                    <div
                      ...
                      style={{
                        width: `${Math.max(
                          0,
                          Math.min(100, dashboard.summary.metrics.workspaceHealth || 0),
                        )}%`,
                      }}
                    />
                  </div>
                  ...
                  <div className="mt-5 grid grid-cols-3 gap-3">
                    <div className="rounded-2xl bg-neutral-50 p-3">
                      <p className="text-[11px] font-bold text-neutral-500">Agents</p>
                    ...
                    <div className="rounded-2xl bg-neutral-50 p-3">
                      <p className="text-[11px] font-bold text-neutral-500">Memory</p>
                      <p className="mt-1 text-lg font-black">
                        {formatNumber(dashboard.summary.metrics.memoryRecords)}
                      </p>
                    </div>
                    <div className="rounded-2xl bg-neutral-50 p-3">
                      <p className="text-[11px] font-bold text-neutral-500">Verify</p>
                      <p className="mt-1 text-lg font-black">
                        {formatNumber(dashboard.summary.metrics.verificationPayloads)}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <section className="space-y-5">
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                  ...
                  <StatCard
                    ...
                    value={formatNumber(dashboard.summary.metrics.pendingApprovals)}
                  ...
                  <StatCard
                    title="Monthly Spend"
                    value={formatMoneyFromCents(dashboard.summary.usage.monthlySpendCents)}
