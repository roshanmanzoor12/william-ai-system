...
function StatusBadge({ status }: { status: ActivityRow["status"] }) {
  ...
  return (
    <span className={`inline-flex rounded-full px-2.5 py-1 text-[11px] font-semibold ${classes[status]}`}>
      {status}
    </span>
  );
}
        ...
        <div className="min-w-0 flex-1 px-0 lg:px-5">
          ...
          <div className="mb-6">
            ...
            <p className="mt-1 text-sm text-zinc-500">
              Stay on top of your agents, monitor progress, and track workspace performance.
            </p>
          </div>
          ...
          <section className="grid grid-cols-1 gap-5 xl:grid-cols-[1.1fr_1fr_1fr]">
            <div className="rounded-[26px] bg-white p-5 shadow-sm ring-1 ring-zinc-100">
              <div className="mb-5 flex items-start justify-between">
                <div>
                  <p className="text-xs font-semibold text-zinc-500">Total Balance</p>
                  <h2 className="mt-2 text-3xl font-black tracking-tight">$689,372.00</h2>
                  <p className="mt-1 text-xs font-bold text-emerald-600">+5% from last month</p>
                </div>
              ...
              <div className="mt-5">
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  {wallets.map((wallet) => (
                    <div key={wallet.label} className="rounded-[18px] bg-[#fafafa] p-3 ring-1 ring-zinc-100">
                      <p className="text-[10px] font-bold uppercase text-zinc-400">{wallet.label}</p>
                      <p className="mt-2 text-sm font-black">{wallet.value}</p>
                      <p className="mt-1 text-[10px] text-zinc-400">{wallet.meta}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {stats.map((stat) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <p className={`text-xs font-semibold ${stat.tone === "orange" ? "text-white/80" : "text-zinc-500"}`}>
                      {stat.title}
                    </p>
                    <MoreHorizontal className={`h-4 w-4 ${stat.tone === "orange" ? "text-white/70" : "text-zinc-400"}`} />
                  </div>
                  <h3 className="mt-5 text-3xl font-black">{stat.value}</h3>
                  <p className={`mt-2 text-xs font-bold ${stat.tone === "orange" ? "text-white/85" : "text-emerald-600"}`}>
                    {stat.change}
                  </p>
                </div>
              ))}
              ...
              <div className="rounded-[26px] bg-white p-5 shadow-sm ring-1 ring-zinc-100">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="min-h-[140px] rounded-[24px] bg-[#171717] p-4 text-white">
                    <div className="flex items-center justify-between">
                      <span className="rounded-full bg-white/10 px-2 py-1 text-[10px] font-bold">Active</span>
                      <CreditCard className="h-5 w-5 text-white/70" />
                    </div>
                    <div className="mt-10">
                      <p className="text-[10px] text-white/50">Card Number</p>
                      <p className="mt-1 text-xs font-bold tracking-widest">**** **** 6762</p>
                    </div>
                  </div>

                  <div className="min-h-[140px] rounded-[24px] bg-gradient-to-br from-[#ff8657] to-[#ff4b2b] p-4 text-white">
                    <div className="flex items-center justify-between">
                      <span className="rounded-full bg-white/20 px-2 py-1 text-[10px] font-bold">Active</span>
                      <Sparkles className="h-5 w-5 text-white/80" />
                    </div>
                    <div className="mt-10">
                      <p className="text-[10px] text-white/70">Card Number</p>
                      <p className="mt-1 text-xs font-bold tracking-widest">**** **** 4356</p>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <table className="w-full min-w-[760px] border-collapse bg-white text-left">
                  ...
                  <tbody>
                    {activities.map((row, index) => (
                      <tr key={`${row.id}-${index}`} className="border-t border-zinc-100 text-xs">
                        <td className="px-4 py-4">
                          <input type="checkbox" className="h-4 w-4 rounded border-zinc-300" defaultChecked={index === 3} />
                        </td>
                        <td className="px-4 py-4 font-bold text-zinc-600">{row.id}</td>
                        <td className="px-4 py-4">
