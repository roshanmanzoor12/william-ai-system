...
# =============================================================================
# Database-Backed Auth Store
# =============================================================================
# Persists users/workspaces/memberships through database.db + the real
# User/Workspace/WorkspaceMembership models (database/models/user.py,
# workspace.py). This file used to carry only an InMemoryAuthStore
# ("Replace later with database-backed ... repositories" was its own
# comment) -- every account/workspace vanished on process restart, and
# nothing was actually enforcing workspace *membership*, just trusting
# whatever the JWT claimed.
#
# Known scope limit: session/refresh-token bookkeeping (below) stays
# in-process. Access/refresh JWTs are self-verifying via HMAC signature
# (TokenService.verify_token), so a restart cannot turn an invalid token
# valid -- it only means the session listing/revocation UI and
# revoked-jti blocklist don't survive a restart or scale across more
# than one worker process. A production multi-instance deployment needs
# this backed by Redis or a `sessions` table; tracked in the final
# repair report as a known follow-up, not silently pretended away.

from database.db import db_manager
from database.models.user import User as _DbUser, UserStatus as _DbUserStatus
from database.models.workspace import (
    Workspace as _DbWorkspace,
    WorkspaceMembership as _DbMembership,
...
def _db_enum_value(value: Any) -> str:
...
class DatabaseAuthStore:
    ...
    # -------------------------------------------------------------------
    # Row -> API record mapping
    # -------------------------------------------------------------------

    @staticmethod
    def _user_record(user: _DbUser) -> UserRecord:
        return UserRecord(
            user_id=user.id,
            email=user.email,
    ...
    @staticmethod
    def _workspace_record(workspace: _DbWorkspace) -> WorkspaceRecord:
        return WorkspaceRecord(
            ...
            created_at=workspace.created_at.isoformat() if workspace.created_at else utc_now(),
            updated_at=workspace.updated_at.isoformat() if workspace.updated_at else utc_now(),
            metadata=workspace.metadata_json or {},
        )

    @staticmethod
    def _membership_record(membership: _DbMembership, plan: str) -> MembershipRecord:
        permissions = membership.permissions or {}
        granted = permissions.get("granted", []) if isinstance(permissions, dict) else []

        return MembershipRecord(
            membership_id=membership.id,
            user_id=membership.user_id,
            workspace_id=membership.workspace_id,
            role=_db_enum_value(membership.role),
            plan=plan,
            permissions=list(granted),
            created_at=membership.created_at.isoformat() if membership.created_at else utc_now(),
            updated_at=membership.updated_at.isoformat() if membership.updated_at else utc_now(),
            is_active=membership.status == _DbMembershipStatus.ACTIVE,
        )
    ...
    def create_user_with_workspace(
        ...
        with db_manager.session_scope() as session:
            existing = session.query(_DbUser).filter(_DbUser.email == normalized_email).first()
            if existing:
                raise ValueError("Email is already registered.")

            combined_hash = hash_password(password)
            # hash_password() embeds algorithm/iterations/salt/hash in one
            # "$"-joined string (see verify_password) -- store it whole in
            # password_hash and keep the parsed salt for the informational
            # password_salt column the User model also carries.
            ...
            db_user = _DbUser(
                ...
                # No outbound-email verification pipeline exists yet
                # (see WILLIAM_SMTP_* in .env.example, all unset by
                # default) -- defaulting new accounts to ACTIVE rather
                # than a PENDING_VERIFICATION state nothing can clear.
                status=_DbUserStatus.ACTIVE,
                is_active=True,
            )
            session.add(db_user)
            session.flush()
            ...
            db_user.default_workspace_id = db_workspace.id

            db_membership = _DbMembership(
                workspace_id=db_workspace.id,
                user_id=db_user.id,
                role=_DbMemberRole.OWNER,
                status=_DbMembershipStatus.ACTIVE,
                permissions={
                    "granted": [
                        "workspace:read",
                        "workspace:update",
            ...
            user_record = self._user_record(db_user)
            workspace_record = self._workspace_record(db_workspace)
            membership_record = self._membership_record(db_membership, plan=workspace_record.plan)

            return user_record, workspace_record, membership_record

    def get_user_by_email(self, email: str) -> Optional[UserRecord]:
        with db_manager.session_scope() as session:
            db_user = session.query(_DbUser).filter(_DbUser.email == normalize_email(email)).first()
            return self._user_record(db_user) if db_user else None

    def get_user_by_id(self, user_id: str) -> Optional[UserRecord]:
        with db_manager.session_scope() as session:
            db_user = session.get(_DbUser, user_id)
            return self._user_record(db_user) if db_user else None

    def get_workspace(self, workspace_id: str) -> Optional[WorkspaceRecord]:
        with db_manager.session_scope() as session:
            db_workspace = session.get(_DbWorkspace, workspace_id)
            return self._workspace_record(db_workspace) if db_workspace else None

    def list_memberships_for_user(self, user_id: str) -> List[MembershipRecord]:
        with db_manager.session_scope() as session:
            rows = (
                session.query(_DbMembership)
                .filter(
                    _DbMembership.user_id == user_id,
                    _DbMembership.status == _DbMembershipStatus.ACTIVE,
                )
                .all()
            )
            ...
            for row in rows:
                workspace = session.get(_DbWorkspace, row.workspace_id)
                plan = _db_enum_value(workspace.plan) if workspace else normalize_plan(AUTH_SETTINGS.default_plan)
                records.append(self._membership_record(row, plan=plan))
...
def write_auth_audit(
    ...
    if AUTH_SETTINGS.audit_enabled:
        ...
        try:
            from database.db import db_manager as _audit_db_manager
            from database.models.security import AuditLogModel as _AuditLogModel

            with _audit_db_manager.session_scope() as _session:
                _session.add(
                    _AuditLogModel(
                        user_id=user_id or "unauthenticated",
                        workspace_id=workspace_id or "unassigned",
                        action=action,
                        resource_type="auth",
                        agent_key="auth_api",
                        actor=user_id or "unauthenticated",
                        status=result,
            ...
            logger.exception("Failed to persist auth audit event to database.")
